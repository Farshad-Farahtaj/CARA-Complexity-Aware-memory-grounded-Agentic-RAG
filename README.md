...
CARA is a multi-tenant medical chatbot platform built as a master's thesis project
(Data Science, Università Federico II di Napoli — Farshad Farahtaj, expected
graduation October 2026). Clinics and hospitals onboard their patients, patients
chat with an AI assistant about their health, and — this is the core idea of the
thesis — CARA knows when a question is too complex or too risky to answer alone
and hands it off to a real doctor instead of guessing.

Every claim below is backed by a script in this repo you can re-run yourself —
see [Reproducing the evaluations](#reproducing-the-evaluations).

🔗 **[Explore the interactive results dashboard →](https://claude.ai/artifact/1SqApXYKy3bcbRssNozCtt)**
— the same three charts below, but hover any point/cell for the real case behind it.
...
- **Patients** register under their clinic (using a Clinic ID their clinic gives
  ...
  allergies into account, and rewrites its own technical answer into warm,
...
This is the thesis's central mechanism. Every answer CARA drafts is passed
through a **safety reviewer** prompt (`needs_doctor_review` in `app.py`) that
judges the *question*, not just whether the drafted answer sounds confident and
well-written. It escalates whenever, for example:
...
1. It's written to the `escalations` table with CARA's draft answer and the
   reviewer's reason.
2. `telegram_bot.py` — a small always-on background process, independent of the
   Streamlit app — polls for pending escalations and sends each one to the
   doctor's Telegram as a message with the patient's relevant history,
   medications, and allergies attached.
...
Everything is backed by a single SQLite database (`backend/cara.db`, managed
entirely through `backend/database.py`). Clinics and patients are separate
...
This replaces the original proposal's 3-tier model router with a single
production model — see [RQ1, below](#research-questions--results) for why —
while keeping the rest of the pipeline (retrieval, grounding, and the
escalation safety net) exactly as designed.

## Research questions & results

### RQ1 — which model actually deserves the production slot

The original proposal called for **complexity-based routing** across three
model tiers (a small/medium/large model, picked per-question by a lightweight
classifier). Before building that, the four realistic open-weight candidates
were benchmarked head-to-head on **100 MedQA questions**
(`backend/evaluate.py` runs the benchmark, `backend/analyze_results.py` builds
the comparison, raw results in `eval_results/`):

![Model comparison — accuracy vs. latency](assets/model_comparison.svg)

**Finding:** GPT-OSS-120B is not a trade-off pick — it wins on accuracy *and*
latency at the same time. A router only earns its complexity if there's a
cost/accuracy trade-off to exploit between tiers; here there isn't one, so the
tiered-routing layer from the proposal was replaced with this single,
empirically-justified model choice. That substitution — and the evidence
above — is the direct answer to RQ1, not a shortcut around it.

### RQ2 — does retrieval (RAG) actually help

50 test questions were generated automatically from real chunks in the RAG
knowledge base (a fixed random seed, one question per chunk, written by the
LLM itself to require that exact passage — see `build_rag_ablation_testset.py`).
Each question was then answered twice — once with retrieval on, once with it
forced off — and an LLM judge checked each answer against the real source
passage, blind to which condition produced it.

![RAG on vs. off, question by question](assets/rag_ablation.svg)

**Finding:** retrieval measurably helps (42% vs. 32% consistent with the
source), even on this deliberately hard, citation-level test set designed to
stress-test grounding rather than reflect typical patient questions. The
per-question view above matters more than the average: retrieval doesn't win
uniformly — it flips some questions in CARA's favor and a few against it,
which is itself useful evidence about where a large pretrained model's own
knowledge competes with retrieval.

### RQ3 — how reliable is the escalation guard

100 test cases were built from real patient records across all three demo
clinics (`build_escalation_testset.py`): six explicit, rule-based categories
mirroring the exact criteria in the guard's own prompt, so every label follows
mechanically from real medication counts, conditions, and allergies rather than
being hand-picked.
...
(false negatives) are the ones worth studying further — they're saved in full,
per-case detail in `escalation_eval_results/escalation_eval_raw.json`.

*(RQ4 — whether human-in-the-loop approval increases trust without adding
friction — is a user-study question this repo doesn't attempt to answer
quantitatively; the approval mechanism itself is built and working, described
above.)*
...
examples, two demo clinics are populated with **real de-identified synthetic
patient records** from the [Synthea](https://synthetichealth.github.io/synthea/)
...
- **Riverside General Hospital** — 200 more patients from a different random
  seed (so the two clinics have genuinely non-overlapping populations, which is
  the point of testing multi-tenant isolation), plus one hand-picked high-risk
  case (Jame Kuhlman — 18 real conditions and 17 medications including
  warfarin, used to stress-test the escalation guard against realistic
...
because the Synthea data has no dental-specific conditions. Several of these
profiles are deliberately designed to probe the escalation logic on real
dental-safety scenarios — a patient on warfarin needing an extraction, a
penicillin allergy alongside a dental abscess, a bisphosphonate raising jaw-bone
risk before an implant, and so on.
